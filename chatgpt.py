#!/usr/bin/python3
import argparse, openai, re, subprocess
...
parser = argparse.ArgumentParser(description='Ask ChatGPT a question, explain a command, or find a command for what you want to do')
...
if not (args.explain or args.ask or args.findcommand):
...
if openai.api_key == None:
    parser.error("Set api key with `export OPENAI_API_KEY=<YOUR_KEY_HERE>`")
...
def sendPrompt(Prompt):
    ...
    if args.ask:
        ...
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Explain terminal commands"},
            {"role": "user", "content": "What does the terminal command touch test.txt do?"},
            {"role": "assistant", "content": "The command `touch test.txt` creates a text file named test.txt in your current directory"},
            {"role": "user", "content": Prompt}
        ]
    )
...
def getCommand(originalString: str) -> str or bool:
